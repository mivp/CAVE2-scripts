...
int draw_persp(unsigned int i, float N, float F, int e, const float p[3])
{
	...
	// andy big stencil buffer setup hack

	//new - doesnt seem to do anything helpful right now
	glPushAttrib(GL_ALL_ATTRIB_BITS);

	// had this here ... seems un-needed
	//	glMatrixMode(GL_MODELVIEW);
	//glLoadIdentity;
	glMatrixMode (GL_PROJECTION);
	glPushMatrix();
	...
	gluOrtho2D(T->win_x, T->win_x+T->win_w-1, T->win_y, T->win_y + T->win_h);
	...
	for (y=T->win_y; y<(T->win_y + T->win_h+1); y +=2)
	  {
	    glLineWidth(1);
	    glBegin(GL_LINES);
	    glVertex2f(T->win_x,            y);
	    glVertex2f(T->win_x + T->win_w, y);
	    glEnd();
	  }
