...
static void get_eye_pos(float d[3], struct camera *c, int eye, int tile)
{
  ...
  //if (get_tile_flags(tile) & TILE_LOCAL_ROT)
	...
	float r0, r1, r2, u0, u1, u2, n0, n1, n2;
	...
	if (r[0] < 0) r0 = -1; else r0 = 1;
	if (r[1] < 0) r1 = -1; else r1 = 1;
	if (r[2] < 0) r2 = -1; else r2 = 1;

	if (u[0] < 0) u0 = -1; else u0 = 1;
	if (u[1] < 0) u1 = -1; else u1 = 1;
	if (u[2] < 0) u2 = -1; else u2 = 1;

	if (n[0] < 0) n0 = -1; else n0 = 1;
	if (n[1] < 0) n1 = -1; else n1 = 1;
	if (n[2] < 0) n2 = -1; else n2 = 1;
	

	// the r u and n are not normalized - should they be?
	
	r[0] = r[0]*r[0] / (r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
	r[1] = r[1]*r[1] / (r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
	r[2] = r[2]*r[2] / (r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);

	u[0] = u[0]*u[0] / (u[0]*u[0] + u[1]*u[1] + u[2]*u[2]);
	u[1] = u[1]*u[1] / (u[0]*u[0] + u[1]*u[1] + u[2]*u[2]);
	u[2] = u[2]*u[2] / (u[0]*u[0] + u[1]*u[1] + u[2]*u[2]);

	n[0] = n[0]*n[0] / (n[0]*n[0] + n[1]*n[1] + n[2]*n[2]);
	n[1] = n[1]*n[1] / (n[0]*n[0] + n[1]*n[1] + n[2]*n[2]);
	n[2] = n[2]*n[2] / (n[0]*n[0] + n[1]*n[1] + n[2]*n[2]);
	

	// another option is to hold onto the sign and re-apply it	
	...
	// if keeping the normalization why no sqrt ?

	// and yet another option is to put the orig if back or 
	// force the if into the else path with no r u n
